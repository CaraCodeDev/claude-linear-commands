...
**IMPORTANT: All tickets are implementation tickets.** Ignore any legacy "planning ticket" patterns. Even if a ticket title says "Plan X" or the description mentions "create implementation tickets" as a deliverable, work it directly. There is no separate planning workflow - just implement what the ticket describes.

## Step 1: Load the Ticket

Fetch the ticket from Linear using `get_issue` and `list_comments` **in parallel** (both calls at once). Extract:
...
3. **Present a summary**
4. **Check for project overview updates** (see below)
5. **Ask for review**
...
During implementation, you may discover things that affect the broader project:
- Scope changes (e.g., "we need 5 permission tiers, not 4")
- Architecture decisions that affect other phases
- New constraints or dependencies discovered
- Approach changes that future tickets should know about

**If you encountered significant learnings**, surface them:

> **Project Overview Update Needed?**
>
> During this work, I discovered [X]. This affects the project because [Y].
>
> Should I update the project overview to reflect this?

If yes, make a **surgical update** to the project description in Linear:
